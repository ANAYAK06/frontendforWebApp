...
function VerifyPCCDcaBudget() {
    ...
    useEffect(() => {
        if (updateSuccess) {
            showToast('success', 'DCA Budget verified successfully')
            closeBudgetDetails()
            dispatch(fetchDCABudgetForVerification({ userRoleId }))
            dispatch(resetUpdateSuccess())
        }
        if (rejectSuccess) {
            showToast('error', 'DCA Budget Rejected Successfully')
            closeBudgetDetails()
            dispatch(fetchDCABudgetForVerification({ userRoleId }))
    ...
    const handleVerify = async () => {
        ...
        try {
            ...
            showToast('error', 'Failed to verify DCA Budget:' + error.message)
    ...
    const handleReject = async () => {
        ...
        try {
            ...
            showToast('error', 'Failed To Reject DCA Budget' + error.message)
    ...
    return (
        <div className='w-full bg-white shadow-md rounded-md overflow-hidden mb-4 mt-4'>
                ...
                <div className='p-4 bg-slate-100 flex justify-between items-center '>
                        ...
                        <div><h3 className='text-gray-600 font-bold'>DCA Budget Verification</h3></div>
